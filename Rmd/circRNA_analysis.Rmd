...
The circRNA annoation is based on mm9, so we need to lift it over to mm10 if we want to compare it with our BSJs. For this, we use the R function `liftOver` from the `rtracklayer` package. The chain file was downloaded from [UCSC](https://hgdownload-test.gi.ucsc.edu/goldenPath/mm9/liftOver/).
...
We compar the annotation with the CIRCexplrer2 results
```{r}
## how many BSJ are already known? (independent of strand and arbitrary overlap)
