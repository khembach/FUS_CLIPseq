...
# gtf_file <- "/Volumes/Shared/data/annotation/Mouse/Ensembl_GRCm38.90/gtf/Mus_musculus.GRCm38.90.gtf"
...
ggsave(here("Rmd", "target_gene_selection_analysis_figures", 
            "MA_pValue.png"), width = 7, height = 5.5)
```
...
ggsave(here("Rmd", "target_gene_selection_analysis_figures", 
            "pValue_cutoffs_barplot.png"), width = 5, height = 5)
```
...
ggplot(lrt$table, aes(x=logCPM, y=logFC, color = PValue < pValue_cut & logFC > 0)) +
...
ggsave(here("Rmd", "target_gene_selection_analysis_figures", 
            "MA_pValue_cutoff_brown3.png"), width = 6.4, height = 5)
...
ggsave(here("Rmd", "target_gene_selection_analysis_figures", 
            "ORA_circle_plot_combined_final.png"), width = 6.5, height = 7.5)
```
...
ggsave(here("Rmd", "target_gene_selection_analysis_figures", 
            "MA_pValue_total_cortex.png"), width = 7, height = 5.5)
```
...
peak_gene_region_barplot(peaks_list_pValue, anno)
ggsave(here("Rmd", "target_gene_selection_analysis_figures", 
            "pValue_cutoffs_barplot_total_cortex.png"), width = 5, height = 5)
```
...
ggplot(lrt$table, aes(x=logCPM, y=logFC, color = PValue < pValue_cut & logFC > 0)) +
  ...
  guides(colour = guide_legend(override.aes = list(alpha = 1, size = 5)))
ggsave(here("Rmd", "target_gene_selection_analysis_figures", 
            "MA_pValue_cutoff_total.png"), width = 6.4, height = 5)
```
