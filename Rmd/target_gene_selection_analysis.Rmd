...
if (!file.exists(here("Rmd", "rds", "peak_counts.RDS"))){
  cts_clip <- data.frame(SNS = counts[["SNS_70K"]]$records, 
...
What are the genes in the line on the top left corner?
```{r MA-plot-top-line}
row_order <- order(lrt$table$logFC, decreasing=TRUE) %>% head
lrt$table[head(row_order),]
## what are the raw counts for this peak?
dge$counts[head(row_order),]
## this are the peaks with 0 reads in the total cortex sample
## They separate from the rest of the points because of the prior count (0.125) that allows us to compute the log2FC for these peaks
```
...
peak_gene_region_barplot <- function(peaks, anno) {
  ...
  print(ggplot(df, aes(x = peaks, y = percentage, fill = annotation)) +
          ...
          scale_x_discrete(breaks=c("all_peaks", "pVal_1em2", "pVal_5em3", 
           ...
           #                              "intron" = "#7BAFDE")) +
           # scale_fill_manual(labels = c("exon", "5' UTR", "3' UTR", "intron"), 
           #                   values = c("exon" = "#B17BA6", 
           #                              "five_prime_utr" = "#F7EE55",
           #                              "three_prime_utr" = "#E68E34", 
           #                              "intron" = "#90C987")) +
           # scale_fill_manual(labels = c("exon", "5' UTR", "3' UTR", "intron"), 
           #                   values = c("exon" = "#9B6EAD", 
...
ggsave(file.path(fig_dir, "fig2_supp", "pValue_cutoffs_barplot_SNS_colors.pdf"), 
       plot = p, width = 5, height = 5)
...
We also save the annotated peaks as an R object.
```{r save-loess-peaks-RDS}
saveRDS(peaks_list_pValue, here("Rmd", "rds", paste0("loess_peaks_sns_list_pValue.RDS")))
saveRDS(loess_peaks, here("Rmd", "rds", paste0("loess_peaks_sns_pValue_", pValue_cut, ".RDS")))
```


We look at some targets with less stringent p-value cutoffs.
```{r targets-less-stringent-cutoff}
loess_peaks_less <- peaks_list_pValue[["pVal_1em4"]]
loess_peaks_less <- add_gene_annotation(loess_peaks_less, anno[["gene"]])

loess_targets_less <- loess_peaks_less %>%
    as.data.frame() %>%
    dplyr::group_by(gene_id) %>%
    dplyr::select(seqnames, gene_id, gene_name, gene_biotype) %>%
    dplyr::mutate(nr_peaks = dplyr::n()) %>%
    dplyr::arrange(desc(nr_peaks)) %>%
    unique()
head(loess_targets_less)

write.table(loess_targets_less,
            file = file.path(out_dir, paste0("top_peaks_loess_adjM_pValue_1e-04_gene_anno.txt")),
            quote = FALSE, row.names = FALSE, sep = "\t")
## which genes are new and not in the list with the most stringent cutoff?
loess_targets_less[!loess_targets_less$gene_id %in% loess_targets$gene_id,] %>% head
write.table(loess_targets_less[!loess_targets_less$gene_id %in% loess_targets$gene_id,],
            file = file.path(out_dir, paste0("top_peaks_loess_adjM_pValue_1e-04_gene_anno_not_in_1e-05.txt")),
            quote = FALSE, row.names = FALSE, sep = "\t")


bed_file <- file.path(out_dir, "selected_peaks", 
                      paste0("selected_peaks_SNS_pValue_1e-04.bed"))
if(!file.exists(bed_file)) {
  export(loess_peaks_less, bed_file)
}
```
...
We manually filtered the genes assigned to each peak (see filter_fUS_targets.html) and removed wrong gene assignments from CLIPper.
We import the filtered list and used it for the following analyses.
...
fus_targets <- read.table(fus_targets, header = TRUE)
```
...
The background are all genes that have cpm > 1 in all SNS WT 1 month replicates.

```{r import-SNS-bg, cache = TRUE}
se <- file.path(rnaseq_dir, "output/outputR/edgeR_dge.rds")
se <- readRDS(se)
## Get gene-level SummarizedExperiment object
sg <- se$sg
rnaseq_meta <- colData(sg)

##log2 counts per million
logcpms <- assay(sg, "logcpm")

## all genes with >= 1 CPM in all replicates per group (logcpm 0 = cpm 1)
sns_bg <- logcpms %>% as.data.frame() %>% 
    dplyr::select(rnaseq_meta[rnaseq_meta$group == "SNS.WT.1_mo", "names"]) %>%
...
```{r convert-ensembl-ID-entrez, cache = TRUE, dependson="import-SNS-bg"}
...
if (file.exists(here("Rmd", "rds", "sns_bg_1month_entrez.rds"))) {
  sns_bg <- readRDS(here("Rmd", "rds", "sns_bg_1month_entrez.rds"))  
} else{
  ## convert to Entrez IDs
  ...
  saveRDS(sns_bg, here("Rmd", "rds", "sns_bg_1month_entrez.rds")) 
}
```


```{r loess-ORA-pValue, cache = TRUE, dependson = c("loess-p-value-different-cutoffs", "convert-ensembl-ID-entrez")}
loess_genes <- unique(fus_targets$gene_id)

if (file.exists(here("Rmd", "rds", "entrez_loess_filtered.rds"))) {
  entrez_loess <- readRDS(here("Rmd", "rds", "entrez_loess_filtered.rds"))  
} else{
  ...
  saveRDS(entrez_loess, here("Rmd", "rds", "entrez_loess_filtered.rds")) 
}
...
```{r write-goana-pValue, include = FALSE, eval = TRUE}
...
```{r ORA-circle-plot-CC, cache = TRUE, dependson="loess-ORA-pValue"}
library(ggsci)
df <- topGO(goana_loess, ontology = "CC", number = 18)
df
## we remove very similar terms (colored terms in Sup. tables fig. S2
df <- df[-c(12, 13, 14, 17),]
df$Term[10] <- "plasma membrane bounded\ncell projection"
df$Term[10] <- "sonatodentritic\ncompartment"

p <- ggplot(df, aes(x = -log10(P.DE), y = log2(DE), size = log2(N), fill = TRUE)) + 
...
p
ggsave(here("Rmd", "target_gene_selection_analysis_figures", 
            "ORA_circle_plot_CC_final.png"), plot = p, width = 6, height = 7)
```

```{r, cache = TRUE, dependson = "ORA-circle-plot-CC"}
ggsave(file.path(fig_dir, "fig2_supp", "ORA_circle_plot_CC_final.pdf"), 
       plot = p, width = 6, height = 7)
```


```{r ORA-circle-plot-BP, cache = TRUE, dependson="loess-ORA-pValue"}
df <- topGO(goana_loess, ontology = "BP", number = 14)
df
## we remove nr 4, because it is identical to 5 and there is one other category with a similar term
df <- df[-c(4, 10, 11, 12),]
df$Term[1] <- "regulation of\nbiological quality"
df$Term[2] <- "trans-synaptic\nsignaling"
df$Term[4] <- "chemical synaptic\ntransmission"
df$Term[10] <- "neuron projection\ndevelopment" 
...
p <- ggplot(df, aes(x = -log10(P.DE), y = log2(DE), size = log2(N), fill = TRUE)) + 
  ...
  theme(text = element_text(size = 20), 
        ...
        legend.box = "horizontal", plot.title = element_text(hjust = 0.5))  +
  geom_text_repel(aes(label=Term), size=4, data=df, color = "black", 
                  point.padding = 0.3, box.padding = 0.5, xlim = c(14.9, NA)) +
  scale_size(breaks=pretty_breaks(4), range = c(3, 13)) +
  guides(size = guide_legend("log2(genes with GO term)", title.position = "top")) +
  labs(x = "-log10(p-value)", y = "log2(target genes with GO term)")+
...
p
ggsave(here("Rmd", "target_gene_selection_analysis_figures", 
            "ORA_circle_plot_BP_final.png"), plot = p, width = 6, height = 7)
```

```{r, dependson = "ORA-circle-plot-BP", cache = TRUE}
ggsave(file.path(fig_dir, "fig2_supp", "ORA_circle_plot_BP_final.pdf"), 
       plot = p, width = 6, height = 7)
```



```{r ORA-circle-plot-MF, cache = TRUE, dependson="loess-ORA-pValue"}
df <- topGO(goana_loess, ontology = "MF", number = 9)
df
## we remove nr 7 and 8, because they are identical to 9 
df <- df[-c(7, 8),]
df$Term[5] <- "inorganic cation transmembrane\ntransporter activity"
...
p <- ggplot(df, aes(x = -log10(P.DE), y = log2(DE), size = log2(N), fill = TRUE)) + 
  ...
  theme(text = element_text(size = 20), 
        ...
        legend.box = "horizontal", plot.title = element_text(hjust = 0.5))  +
  geom_text_repel(aes(label=Term), size=4, data=df, color = "black", 
                  point.padding = 0.3, box.padding = 0.2, xlim = c(9.2, NA)) +
  scale_size(breaks=pretty_breaks(4), range = c(3, 13)) +
  guides(size = guide_legend("log2(genes with GO term)", title.position = "top")) +
  labs(x = "-log10(p-value)", y = "log2(target genes with GO term)") +
...
p
ggsave(here("Rmd", "target_gene_selection_analysis_figures", 
            "ORA_circle_plot_MF_final.png"), plot = p, width = 6, height = 7)
```

```{r, dependson = "ORA-circle-plot-MF", cache = TRUE}
ggsave(file.path(fig_dir, "fig2_supp", "ORA_circle_plot_MF_final.pdf"), 
       plot = p, width = 6, height = 7)
```
...
```{r ORA-circle-plot-combined, cache = TRUE, dependson="loess-ORA-pValue"}
df_comb <- rbind(topGO(goana_loess, ontology = "CC", number = 18)[-c(seq(5,14), 16, 17),], 
                 topGO(goana_loess, ontology = "BP", number = 9)[-seq(3, 7),],
                 topGO(goana_loess, ontology = "MF", number = 5)[-c(2, 4),])
df_comb$Term[10] <- "estabilishment\nof localization"
df_comb$Term[8] <- "trans-synaptic\nsignaling"
df_comb$Term[13] <- "inorganic cation transmembrane\ntransporter activity"
...
p <- ggplot(df_comb, aes(x = -log10(P.DE), y = log2(DE), size = log2(N), fill = Ont)) + 
  ...
  geom_text_repel(aes(label=Term), size=5, data=df_comb, color = "black", 
                  point.padding = 0.3, box.padding = 0.4, xlim = c(9.7, 41)) +
  scale_size(breaks=pretty_breaks(4), range = c(3, 13)) +
  guides(size = guide_legend("log2(genes with GO term)", title.position = "top"),
...
# Max peak coverage vs. gene log2cpms from RNA-seq SNS 1 month
...
```{r max-peak-coverage-all, cache = TRUE, dependson = c("CLIP-coverage", "import-SNS-bg")}
...
## compute the mean logcpms of the RNA-seq samples per peak
logcpm_per_peak <- logcpms %>% as.data.frame %>%
  dplyr::select(starts_with("1_mo_WT_SNS")) %>%
  tibble::rownames_to_column("gene_id") %>%
  mutate(mean_logcpm = rowMeans(dplyr:: select(., -gene_id))) %>% 
  dplyr::select(c(gene_id, mean_logcpm)) %>%
  tidyr::separate(gene_id, into = c("gene_id", "gene_name"), sep = "__")

## merge the peak df with the tpms
df_peak_cov <- df_peak_cov %>%
  dplyr::left_join(., logcpm_per_peak, by = c("gene_id")) %>%
    dplyr::filter(!is.na(mean_logcpm))
...
plot_peak_strength_cpm <- function(df, y_col = "max_coverage", 
  ...
  p <- ggplot(df[!df[,c(col)],], aes_string(x = "mean_logcpm", y = y_col)) +
...
df_peak_cov <- df_peak_cov
df_peak_cov$mean_logcpm <- df_peak_cov$mean_logcpm + 0.01

plot_peak_strength_cpm(df_peak_cov, y_col = "max_coverage", log = FALSE, 
                       col = "selected_peak", cutoff = pValue_cut)
ggsave(here("Rmd", "target_gene_selection_analysis_figures", 
            "peak_max_coverage_vs_mean_cpm.png"), width = 6.5, height = 5)


plot_peak_strength_cpm(df_peak_cov, y_col = "mean_coverage", log = FALSE, 
                       col = "selected_peak", cutoff = pValue_cut)
ggsave(here("Rmd", "target_gene_selection_analysis_figures", 
...
# Max peak coverage in introns and exons
What is the distribution of the maximal number of reads in intronic and exonic peaks?
```{r max-reads-intron-exon}
## get all intronic peaks
intr_peaks <- subsetByOverlaps(loess_peaks, anno[["intron"]])
length(intr_peaks)
exon_peaks <- subsetByOverlaps(loess_peaks, anno[["intron"]], invert = TRUE)
length(exon_peaks)
gg_dat <- data.frame(max_cov = c(max(cov[["SNS_70K"]][intr_peaks]), 
                       max(cov[["SNS_70K"]][exon_peaks])),
           location = c(rep("intron", length(intr_peaks)), 
                        rep("exon", length(exon_peaks))))
p <- ggplot(gg_dat, aes(x = location, y = max_cov)) +
  geom_violin(alpha = 0.4) +
  geom_boxplot(alpha = 1, outlier.shape = NA, width=0.3, fill = "white") + 
  scale_y_log10() +
  theme_light(base_size = 16) + 
  guides(alpha = FALSE, fill = FALSE) +
  # facet_grid(vars(measure), scale = "free_y") +
  xlab("peak location") + ylab("maximal peak height (# reads)") +
...
ggsave(file.path(fig_dir, "fig2_supp", "boxplot_max_cov_intron_exon_peak.png"), 
        plot = p, width = 5, height = 5, type = "cairo", dpi = 500)


## median coverage
gg_dat <- data.frame(max_cov = c(median(cov[["SNS_70K"]][intr_peaks]), 
                       median(cov[["SNS_70K"]][exon_peaks])),
           location = c(rep("intron", length(intr_peaks)), 
                        rep("exon", length(exon_peaks))))
p <- ggplot(gg_dat, aes(x = location, y = max_cov)) +
  geom_violin(alpha = 0.4) +
  ...
  guides(alpha = FALSE, fill = FALSE) +
  # facet_grid(vars(measure), scale = "free_y") +
  xlab("peak location") + ylab("median peak height (# reads)") +
  # scale_fill_manual(values = c("#117733", "#882255")) +
  theme(strip.background = element_rect(fill = "white"), 
      strip.text.x = element_text(colour = "black", face = "bold"),
      strip.text.y = element_text(colour = "black", face = "bold"),
...
ggsave(file.path(fig_dir, "fig2_supp", "boxplot_max_cov_intron_exon_peak_median.png"), 
        plot = p, width = 5, height = 5, type = "cairo", dpi = 500)


## what if we remove the intronic peaks that overlap both introns and exons?
intr_only_peaks <- subsetByOverlaps(loess_peaks, anno[["intron"]], type = "within")
length(intr_only_peaks)
not_intron_peaks <- subsetByOverlaps(loess_peaks, intr_only_peaks, type = "equal", invert = TRUE)
length(not_intron_peaks)
gg_dat <- data.frame(max_cov = c(max(cov[["SNS_70K"]][intr_only_peaks]), 
                       max(cov[["SNS_70K"]][not_intron_peaks])),
           location = c(rep("intron", length(intr_only_peaks)), 
                        rep("exon", length(not_intron_peaks))))
p <- ggplot(gg_dat, aes(x = location, y = max_cov)) +
  geom_violin(alpha = 0.4) +
  geom_boxplot(alpha = 1, outlier.shape = NA, width=0.3, fill = "white") + 
  scale_y_log10() +
...
ggsave(file.path(fig_dir, "fig2_supp", "boxplot_max_cov_within_intron_exon_peak.png"), 
        plot = p, width = 5, height = 5, type = "cairo", dpi = 500)
```
...
We order the peaks based on the p-values and pick the same number as SNS specific peaks.
...
ggsave(file.path(fig_dir, "fig2_supp", "pValue_cutoffs_barplot_total_color.pdf"), 
       plot = p, width = 5, height = 5)
```
...
ggsave(here("Rmd", "target_gene_selection_analysis_figures", 
            "pValue_single_cutoff_barplot_selected_SNS_total.png"), 
       width = 5, height = 5)
```

```{r}
ggsave(file.path(fig_dir, "fig2", "pValue_cutoff_barplot_selected_SNS_total_color.png"), 
       plot = peak_gene_region_barplot_subs_selected(subs, anno), 
