...
```{r write-BED, cache = TRUE}
loess_peaks$name <- loess_peaks$gene_id
...
```{r write-goana-pValue, cache = TRUE}
...
```{r pre-postsynaptic-genes, cache = TRUE}
...
![Slc1a2](../analysis/deduplicated/MA_plot_selection/IGV_pictures/top_pValue/Slc1a2.png)

$~$

SPARC-like protein 1:
secreted protein with high structural similarity to SPARC
![Sparcl1](../analysis/deduplicated/MA_plot_selection/IGV_pictures/top_pValue/Sparcl1.png)
...
![Chgb](../analysis/deduplicated/MA_plot_selection/IGV_pictures/top_pValue/Chgb.png)

Two genes with only a single peak.

$~$

erythroid differentiation regulator 1
![Erdr1](../analysis/deduplicated/MA_plot_selection/IGV_pictures/top_pValue/Erdr1.png)


$~$

glutamate receptor, ionotropic, AMPA3 (alpha 3)
![Gria3](../analysis/deduplicated/MA_plot_selection/IGV_pictures/top_pValue/Gria3.png)
...
## colored by selected peaks (p-value <0.0001)
sns_targets <- paste(seqnames(loess_peaks), start(loess_peaks), 
                 end(loess_peaks), sep = "_")
df_logfc$target <- sns_ids %in% sns_targets

ggplot(df_logfc, aes(x = logFC_CLIP, y = logFC_RNAseq)) + 
  geom_point(alpha = 0.2, aes(color = target)) + 
  theme_bw() 
